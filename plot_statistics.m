...
for x=1:N
    ...
    drawnow
    hold on;
    plot(traj_single(x,:),tt_single,'Color',[0.9290 0.6940 0.1250]);
    drawnow
    hold on;
end
hold off;
...
ylim([0 time])
legend({'photon','noninteracting electron','single electron w/ boundary'},'Location','southwest');
xlimits=xlim;
ylimits=ylim;
...
figure(4)
histogram(yy_photon_alice);
hold on
histogram(yy_electron_alice);
hold on
histogram(yy_single);
...
figure(5)
scatter(initvals(1,:),n_yy_photon_alice);
hold on;
scatter(initvals(2,:),n_yy_electron_alice);
hold on;
scatter(initvals(2,:),n_yy_single);
hold on;
xlabel('Initial position','FontSize',20);
ylabel('Arrival time','FontSize',20);
...
figure(6)
cdfplot(n_yy_photon_alice);
hold on
cdfplot(n_yy_electron_alice);
hold on
cdfplot(n_yy_single);
...
figure(7)
histogram(n_yy_photon_alice);
hold on
histogram(n_yy_electron_alice);
hold on
histogram(n_yy_single);
...
hold on
plot(times,cumtrapz(times,mu_single));
