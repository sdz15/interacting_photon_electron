...
fun = zeros(time/mesh+1,(mu_el-mu_ph)/mesh+1);
mu_fun = zeros(1,time/mesh+1);
...
for t = 1:time/mesh+1
    for s = 1:(pos_bob-pos_alice)/mesh+1
        ...
        % FIRST TWO TERMS OF (31) FROM ARRIVAL TIMES PAPER
        fun(t,s) = abs(psiPlusMinusNear(times(t),pos_alice,times(t),space(s),theta_ph,theta_el,sigma_ph,sigma_el,k_ph,k_el,mu_ph,mu_el,omega,ang)).^2+abs(psiPlusPlusNear(times(t),pos_alice,times(t),space(s),theta_ph,theta_el,sigma_ph,sigma_el,k_ph,k_el,mu_ph,mu_el,omega,ang)).^2;
    end
    mu_fun(t) = trapz(fun(t,:))*mesh;
end
...
title('Trajectories for far configuration');
...
figure(4)
cdfplot(yy_photon);
title('Cumulative distribution of arrival time of photon at Alice');
xlim([0 time])
text(time-.5,.5,txt);

figure(5)
histogram(yy_photon,ceil(N/5));
title('Histogram of arrival time of photon at Alice');
xlim([0 time])
text(time-.5,10,txt);
...
xlim([0 time])
plot(times,mu_fun);
title('Probability density function of arrival time of photon at Alice')
text(time-.5,.5,txt);

figure(7)
xlim([0 time])
plot(times,cumtrapz(times,mu_fun));
title('Cumulative distribution function of arrival time of photon at Alice')
text(time-.5,.1,txt);
