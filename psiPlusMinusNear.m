function val = psiPlusMinusNear(t_ph,s_ph,t_el,s_el,theta_ph,theta_el,sigma_ph,sigma_el,k_ph,k_el,mu_ph,mu_el,omega,ang,step)
    ...
    y3 = trapz(X_c,besselfun((t_el+s_til).*(t_el-s_til-(2*X_c)),omega,1).*exp(-1i.*ang).*psiMinusPlusR(s_ph+t_ph-(2*X_c),s_ph+t_ph-(2*X_c),theta_ph,theta_el,sigma_ph,sigma_el,k_ph,k_el,mu_ph,mu_el));
