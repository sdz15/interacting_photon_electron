...
for x=1:N
    q0 = initvals(:,x);
    [tt_single_free,qq_single_free] = ode45(@(t,q) velocity_single_electron(abs(phiMinusApprox(t,q,sigma_el,kappa,0,omega)).^2,abs(phiPlusApprox(t,q,sigma_el,kappa,0,omega)).^2),times,q0(2)-mu_el);
...
for t = 1:time/mesh+1
    mu_fun_single_free(t) = -2*j1(abs(phiMinusApprox(times_prob(t),L,sigma_el,kappa,0,omega)).^2,abs(phiPlusApprox(times_prob(t),L,sigma_el,kappa,0,omega)).^2);
...
for x=1:N
    ...
    plot(traj_single_free(x,:),tt_single_free);
    drawnow
